    ...
    public function getData()
    {
        ...
        return Datatables::of($roles)
            ->edit_column('created_at', function($row) {
                return $row->created_at->format('M jS, Y h:i');
            })
            ->edit_column('users', '{{{ DB::table(\'role_user\')->where(\'role_id\', \'=\', $id)->count()  }}}')
            ->add_column('actions', '<a href="{{{ URL::to(\'admin/roles/\' . $id . \'/edit\' ) }}}" class="iframe btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
                                <a href="{{{ URL::to(\'admin/roles/\' . $id . \'/delete\' ) }}}" class="iframe btn btn-xs btn-danger"><i class="fa fa-trash"></i></a>
            ')
            ->remove_column('id')
            ->make();
    }
