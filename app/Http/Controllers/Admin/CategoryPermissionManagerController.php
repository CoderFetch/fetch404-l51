...
class CategoryPermissionManagerController extends Controller {
	...
	public function edit(Category $category)
	{
		...
		$accessCategoryIds = CategoryPermission::whereCategoryId($category->id)->where('permission_id', '=', 20)->lists('role_id', 'role_id');
		$createThreadIds = CategoryPermission::whereCategoryId($category->id)->where('permission_id', '=', 1)->lists('role_id', 'role_id');
		$replyIds = CategoryPermission::whereCategoryId($category->id)->where('permission_id', '=', 6)->lists('role_id', 'role_id');
	...
	public function update(UpdateCategoryPermissionsRequest $request)
	{
		...
		CategoryPermission::whereCategoryId($category->id)
